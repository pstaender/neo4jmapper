...
describe 'Neo4jMapper', ->
  ...
  describe 'graph', ->
    ...
    it 'expect to get a neo4j version value', (done) ->
      graph = new Graph()
      graph.about ->
        expect(graph.version).to.be.a 'number'
        expect(graph.version).to.be.above 1.6
        expect(graph.exact_version).to.be.a 'string'
        done()
    ...
    it 'expect to get all nodes', (done) ->
      ...
      n.stream (err,data) ->
        ...
        n.stream (err,found) ->
          ...
          # console.log _trim n.toCypherQuery()
          n.exec ->
            done()
